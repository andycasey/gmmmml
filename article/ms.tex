...
derived the bounds
\begin{equation}
-K\log{K} \quad \leq \quad \sum_{k=1}^{K}\log\weight_k \quad \leq \quad - K\log{K} + \log(N + 1 - K)
\end{equation}
...
\begin{equation}
    |\cov_{l}| \approx \prod_{d=1}^{D}\frac{d_{min}^2}{4} \quad ,
\end{equation}
...
\begin{eqnarray}
    \log{|\cov_{l}|} & \approx & \sum_{d=1}^{D}\log{\frac{d_{min}^2}{4}}  \nonumber \\
...
data. A lower limit, however, does exist.
