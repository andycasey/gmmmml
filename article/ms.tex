...
that describes the uniformity of the mixture from Equation \ref{eq:slw_nonuniform} ($f_u = 0$) to Equation \ref{eq:slw_uniform} ($f_u = 1$). We use estimates of $f_u$ from previously trialled mixtures to make predictions for future, untrialled mixtures.
...
The \emph{Message-Jumping Method} seeks to find the mixture parameters $\vec\psi$ that produce the minimum message length over the class of finite mixtures $\vec{\mathcal{M}}$ in the fewest number of model evaluations.
...
To evaluate the proposed search algorithms, we conduct a set of experiments on synthetic and real datasets. 
